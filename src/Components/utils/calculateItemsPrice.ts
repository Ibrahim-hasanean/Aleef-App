...
export default async function caculateItemsPrice(orderItems: OrdersItemsInterface[]) {
    ...
    for (let i = 0; i < orderItems.length; i++) {
        ...
        if (item?.avaliableQuantity && item.avaliableQuantity <= 0) throw new Error(`this item ${itemId} is out of stock`);
        if (item?.avaliableQuantity && item.avaliableQuantity < orderItem.count) throw new Error(`this item is out of stock`);
        ...
        item.avaliableQuantity = (item?.avaliableQuantity ?? 0) - (1 * orderItem.count);
